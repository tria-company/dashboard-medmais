...
/** Cor dos estados com menos colaboradores (mapa majoritariamente cinza). */
const COR_CINZA = "#F6F6F6";
/** Borda dos estados (contorno visível como na referência). */
...
export default function HeadcountEstado(): React.ReactElement {
  ...
  const { minVal, maxVal, threshold } = useMemo(() => {
    const vals = Object.values(headcountPorEstado).filter(Number.isFinite);
    const min = Math.min(...vals, 0);
    const max = Math.max(...vals, 1);
    const sorted = [...vals].sort((a, b) => a - b);
    const p70Index = Math.floor(sorted.length * 0.7);
    const thresholdVal = sorted[p70Index] ?? min;
    return {
      minVal: min,
      maxVal: max,
            ...
            <Geographies geography={GEO_URL}>
              {({ geographies }) =>
                geographies.map((geo) => {
                  const sigla = (geo.properties?.sigla ?? "").toUpperCase();
                  const valor = headcountPorEstado[sigla] ?? 0;
                  const comCor = valor >= threshold;
                  const fill = comCor
                    ? getCorPorValor(valor, threshold, maxVal)
                    : COR_CINZA;
                  return (
                    <Geography
                      key={geo.rsmKey}
