...
import { trainingBreakdownDonut } from "@/lib/mock/training";

const chartData = trainingBreakdownDonut.map((d) => ({
  name: d.label,
  value: d.value,
  fill: d.color,
}));
...
export default function TrainingBreakdownDonutCard(): React.ReactElement {
  return (
    <CardBase className="flex flex-col" compact>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        <div className="relative mx-auto h-[140px] w-full max-w-[140px] shrink-0 md:mx-0">
        ...
        <ul className="flex flex-1 flex-col gap-2">
          {trainingBreakdownDonut.map((item) => (
            <li
              key={item.label}
