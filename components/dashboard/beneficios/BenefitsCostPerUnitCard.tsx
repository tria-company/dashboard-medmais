...
import { benefitsCostPerUnitData } from "@/lib/mock/beneficios";
...
function CustomTooltip({
  ...
  active?: boolean;
  payload?: Array<{ payload: { unidade: string; valor: number } }>;
  ...
  return (
    <div
      ...
      role="tooltip"
    >
      <p className="mb-1 font-medium text-gray-800">{p.unidade}</p>
...
export default function BenefitsCostPerUnitCard(): React.ReactElement {
  const maxVal = Math.max(...benefitsCostPerUnitData.map((d) => d.valor));
      ...
      <div className="h-[320px] w-full min-w-0">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar
              dataKey="valor"
              fill="url(#benefits-cost-per-unit-bar)"
              radius={[0, 6, 6, 0]}
              maxBarSize={28}
              name="Custo"
            />
